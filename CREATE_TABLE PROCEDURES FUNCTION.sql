...
select count(*) from bank_details where age=60;
...
select job,count(*) as people from bank_details group by job;
...
select job,count(*) from bank_details where balance<0;

select job,balance,housing from bank_details where housing='no';
select job,count(*) from bank_details where housing='no';
select job,marital,housing from bank_details where housing='no';
...
DELIMITER &&
create procedure avg_bal_jobrole(IN sudh varchar(50))
BEGIN 
...
DELIMITER &&
create procedure select_edu_jobs(in v1 varchar(30), in v2 varchar(50))
BEGIN 
	select * from bank_details where education =v1 and job=v2;
END &&

call select_edu_jobs('tertiary','retired');
